...
const std::string ptext =
    ...
    "nose were freezing and he had not thought his fingers could go lifeless "
    "in so short a time lifeless they were for he could scarcely make them "
    "move together to grip a twig and they seemed remote from his body and "
    "from him when he touched a twig he had to look and see whether or not he "
    "had hold of it the wires were pretty well down between him and his finger "
    "ends all of which counted for little there was the fire snapping and "
    ...
    "sheaths of iron half way to the knees and the mocassin strings were like "
    "rods of steel all twisted and knotted as by some conflagration";

size_t alphabet_size = 27;
const double alphabet_ioc = 0.78;
    ...
    for (size_t i = 1; i < groups.size(); i++) {
        if constexpr (true) {
            ...
            for (size_t s = 1; s < alphabet_size; s++) {
                ...
                auto mioc = mutual_index_of_cons(groups[0],
                                                 cesar_encrypt(groups[i], s));
                if (mioc > maxMioc) {
                  maxMioc = mioc;
                  maxS = s;
                }
            ...
            auto sm = value_sorted(shiftsMioc);
            shifts[i] = sm[0].first;
        }
    }
...
double keylength_rank(const std::string& text, size_t keylength)
{
    std::vector<std::string> groups(keylength);
    for (size_t i = 0; i < text.size(); i++) {
        groups[i % groups.size()].push_back(text[i]);
    }

    double rank = 0.0;
    for (const auto& group : groups) {
        rank += 1.0 - abs(index_of_cons(group) - alphabet_ioc);
    }
    rank /= groups.size();
...
size_t find_keylength(const std::string& ctext)
{
    ...
    auto keylength = gcds[0].first;
    auto maxRank = keylength_rank(ctext, keylength);
    auto size = gcds.size() > 5 ? 5 : gcds.size();
    for (size_t i = 1; i < size; i++) {
        auto rank = keylength_rank(ctext, gcds[i].first);
        if (rank > maxRank) {
            keylength = gcds[i].first;
            maxRank = rank;
        }
    }
    return keylength;
...
int main()
{
    ...
    variants["khan"] = "klkbnqlcytfysryucocphgbdizzfcmjwkuchzyeswfogmmetwwossdchrzyldsbwnydednzwnefydthtddbojicemlucdygicczhoadrzcylwadsxpilpiecskomoltejtkmqqymehpmmjxyolwpeewjckznpccpsvsxauyodhalmriocwpelwbcniyfxmwjcemcyrazdqlsomdbfljwnbijxpddsyoehxpceswtoxwbleecsaxcnuetzywfn";
    // for (const auto& dirEntry : recursive_directory_iterator("./variants")) {
    //     std::ifstream text(dirEntry);
    //     std::stringstream buffer;
    //     buffer << text.rdbuf();
    //     variants[dirEntry.path()] = buffer.str();
    // }

    for (auto& [name, txt] : variants) {
        auto keylength = find_keylength(txt);
        auto keys = find_keys2(txt, keylength);
        std::cout << name << ": " << std::endl;
        for (auto& key : keys) {
            auto dec = vig_decrypt(txt, key);
            printf("  %s => %s\n", key.data(), std::string(dec.data(), 64).data());
        }
    }
